...
function getMinRangYear(){
	$fecha = date('Y')-50;
	return $fecha;		
}
function getMaxRangYear(){
	$fecha = date('Y')+50;
	return $fecha;		
}
?>
...
$(function($){
  ...
  function initDatepicker(){
	...
	if (document.documentElement.clientWidth > 764) {
		...
		//destroy datepicker
		$( "#from" ).datepicker({
          yearRange: '<?php echo getMinRangYear().':'.getMaxRangYear(); ?>',
		  dateFormat: 'yy-mm-dd',
		  showButtonPanel: true,
		  changeMonth: true,
		  numberOfMonths: 1,
  ...
  <?php if(get_http_response_code() == 200 || get_http_response_code() == 403){  ?>
  
/********************************************************************************************/
/************************************API REST Analityc <?php echo get_http_response_code() ?>***************************************/
/********************************************************************************************/
    ...
    function visitor(){
        if(data_country.getField('statusCode') == 'OK'){
            if(!visited && localStorage.getItem('object_id')!=null){
            	...
            	console.count(localStorage.getItem('object_id'));
            }
        }else{
            data_country.checkcookie(callback);
        }
