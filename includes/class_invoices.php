...
class Invoices extends TableGateway {
    ...
    */
    public function insert($data) {
        ...
        if($data['next_due_date'] != '0000-00-00' AND isset($data['next_due_date'])) {
    ...
    */
    public function recalculatePrice($invoice_id) {
        ...
        if(!$invoice = $this->db->GetRow("SELECT id, subtotal, tax_rate, tax_rate2, discount_code_value, discount_code_discount_type FROM ".T_INVOICES." i WHERE id=?",array($invoice_id))) {
            return false;
        }
    ...
    */
    public function changeStatus($id, $status) {
        ...
        if($status == 'paid') {
            ...
            $this->db->Execute("UPDATE ".T_INVOICES." SET status=?, date_paid='0000-00-00' WHERE id=?",array($status,$id));
        }
    }
    ...
    */
    public function getPDF($id, $output = false) {
        ...
        $pdf->SetHeaderData('', 0, $this->PMDR->getConfig('invoice_company'), '');
        ...
        if($invoice['tax_rate'] != 0.00) {
            $pdf->MultiCell(120, 0, $this->PMDR->getLanguage('invoices_tax').' ('.$invoice['tax_rate'].'%): ', 0, 'R', 0, 0, '', '', true, 0);
            $pdf->MultiCell(40, 0, format_number_currency($invoice['tax']), 1, 'L', 0, 1, '', '', true, 0);
        }

        if($invoice['tax_rate2'] != 0.00) {
            $pdf->MultiCell(120, 0, $this->PMDR->getLanguage('invoices_tax').' ('.$invoice['tax_rate2'].'%): ', 0, 'R', 0, 0, '', '', true, 0);
