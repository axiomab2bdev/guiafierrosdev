...
class SearchListingFullText extends Search {
    ...
    var $sortBy = array('priority'=>'DESC','score'=>'DESC','l.title'=>'ASC');    // Sort order array, keys are the field to sort by, with their value being the direction
    var $sortByDistance = array('zip_distance'=>'ASC','priority'=>'DESC','score'=>'DESC','l.title'=>'ASC');
    ...
    */
    function applyBooleanFormatting($string) {
        ...
        preg_match_all('/"[^"]+"/', $string, $words_array1);
    ...
    */
    function parseForLIKE($string, $fields) {
        ...
        if(!is_array($string)) {
            preg_match_all('/"([^"]+)"/', $string, $quoted_words);
    ...
    */
    function parseForFulltext($string, $fields, $title = null, $useQueryExpansion = 0) {
        ...
        if($this->titleWeight AND !is_null($title)) {
    ...
    private function extractShortWords(&$string) {
        ...
        if(preg_match_all('/(^|\s)(\S{'.$this->likeShortWordMin.','.$this->likeShortWordMax.'})(?=$|\s)/', $string, $found_short_words)) {
            $string = trim(preg_replace('/(^|\s)(\S{1,'.$this->likeShortWordMax.'})(?=$|\s)/','',$string));
    ...
    function buildQuery($count = false) {
        ...
        if(!empty($this->listing_id)) {
            ...
            if(!empty($this->location_parsed)) {
                ...
                if(!$this->zipLookup OR $this->zipMiles == '') {
                    ...
                    if($this->locationTryExactMatch) {
                        if($found_location = $this->db->GetAll("SELECT id, title FROM ".T_LOCATIONS." WHERE title IN (".$this->PMDR->get('Cleaner')->clean_db($this->location_parsed).",'".implode("','",preg_split('#\W+#',$this->PMDR->get('Cleaner')->clean_db($this->location_parsed),-1,PREG_SPLIT_NO_EMPTY))."') ORDER BY level DESC")) {
            ...
            if(count($this->stripList)) {
                $this->keywords_parsed = preg_replace('/(^|\s)('.implode('|',$this->stripList).')(?=$|\s)/u','',$this->keywords_parsed);
            }
            ...
            if($this->forceLIKE OR ($found_short_words = preg_match_all('/(^|\s)(\S{1})(?=$|\s)/', $this->keywords_parsed,$matches_test)) > 1)  {
                ...
                if(is_numeric($this->category_parsed) AND $this->findMatches) {
                    $this->matchingCategories = $this->db->GetAll("SELECT id, title, friendly_url, friendly_url_path, count_total FROM ".T_CATEGORIES." WHERE ".($this->PMDR->getConfig('cat_empty_hidden') ? 'count_total > 0 AND' : '')." ".$this->parseForLIKE($this->keywords_parsed,$this->matchingCategoriesFields));
                }
            } else {
                ...
                if(!is_numeric($this->category_parsed) AND $this->findMatches) {
                    if(trim($this->category_parsed) != '') {
                        if($category_id = $this->db->GetRow("SELECT id, title, friendly_url, friendly_url_path, count_total FROM ".T_CATEGORIES." WHERE count_total > 0 AND title=?",$this->category_parsed)) {
                        ...
                        if(isset($category_where)) {
                            $this->matchingCategories = $this->db->GetAll("SELECT id, title, friendly_url, friendly_url_path, count_total FROM ".T_CATEGORIES." WHERE ".($this->PMDR->getConfig('cat_empty_hidden') ? 'count_total > 0 AND' : '')." ".$category_where);
                        }
                    }
                }
            ...
            if(is_numeric($this->category_parsed)) {
            ...
            if(!empty($this->zipMiles)) {
                if(!is_null($this->latitude) AND !is_null($this->longitude)) {
                    ...
                    if(!empty($this->zip)) {
                        ...
                        $location_parsed_formatted = '+'.implode(' +',preg_split('/\W+/',$this->location_parsed));
