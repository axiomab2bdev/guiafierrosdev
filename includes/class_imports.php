...
class Imports extends TableGateway{
    ...
    */
    function delete($id) {
        ...
        parent::delete($id);
        unlink(TEMP_UPLOAD_PATH.'import_'.$id.'.csv');
        unlink(TEMP_UPLOAD_PATH.'import_'.$id.'_log.csv');
    ...
    function run($id) {
        ...
        while($count < $import['rate'] AND ($data = fgetcsv($handle, 0, $import['delimiter'], $import['encapsulator'])) !== false) {
                    ...
                    foreach($category as $level=>$category_name) {
                        ...
                        if(isset($category_cache[md5($category_name.$category_id.($level+1))])) {
                            ...
                            $new_category = array(
                                ...
                                'link'=>strval($input['category_link'.($level+1)]),
                                'import_id'=>$id
                            );
                ...
                if(is_numeric($location)) {
                    ...
                    if(isset($location_cache[md5($location.$current_location_id.$level)])) {
                        ...
                        $new_location = array(
                            ...
                            'link'=>strval($input['location_link'.($level)]),
                            'import_id'=>$id
                        );
            ...
            if(trim($input['title']) == '') {
                ...
                if(!isset($input['pricing_id']) OR trim($input['pricing_id']) == '') $input['pricing_id'] = $import['data']['pricing_id'];
        ...
        $this->PMDR->get('Imports')->update(array('data'=>serialize($import['data']),'error_count'=>$import['data']['statistics']['errors'],'rate'=>$new_rate,'position'=>$current,'position_line'=>($import['position_line']+$count),'date_activity'=>$this->PMDR->get('Dates')->dateTimeNow()),$import['id']);
