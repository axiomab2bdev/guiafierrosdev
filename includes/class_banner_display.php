    ...
    */
    function getRecord($type) {
        ...
        if($this->category > 1 OR $this->location > 1) {
            ...
            // If no banner was found and we have the setting to det a random banner, get a random banner
     ...
     function getBanner($type, $remote = false) {
        ...
        if($banner['extension'] != '' AND $banner_url = get_file_url_cdn(BANNERS_PATH.$banner['id'].'.'.$banner['extension'])) {
            if(strtolower($banner['extension']) ==  "swf") {
                $banner_content = $this->PMDR->getNew('Template',PMDROOT.TEMPLATE_PATH.'blocks/banners_swf.tpl');
                ...
                $img_code = '<img id="banner'.$banner['id'].'" class="banner_type'.$banner_type['id'].'" src="'.get_file_url_cdn(BANNERS_PATH.$banner['id'].'.'.$banner['extension']).'?random='.filemtime(BANNERS_PATH.$banner['id'].'.'.$banner['extension']).'" width="'.$banner_type['width'].'" height="'.$banner_type['height'].'" border="0" alt="'.($banner['alt_text'] != '' ? $banner['alt_text'] : $banner['title']).'" />';
                if($link != '') {
