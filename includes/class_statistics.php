    ...
    */
    function getStatistics($type,$listing_id,$date_start=null,$date_end=null) {
        ...
        if($this->getGroupType($type) == 'daily') {
            $statistics = $this->db->GetAll("SELECT date, type, SUM(count) AS count FROM ".T_STATISTICS." WHERE type_id=? $date_sql GROUP BY date, type ORDER BY date ASC",array($listing_id));
            ...
            $statistics = $this->db->GetAll("SELECT IFNULL(MONTH(date),0) AS month, IFNULL(YEAR(date),0) AS year, type, SUM(count) AS count FROM ".T_STATISTICS." WHERE type_id=? $date_sql GROUP BY month, year, type ORDER BY month DESC, year DESC",array($listing_id));
            foreach($statistics AS $statistic) {
