...
class Orders extends TableGateway {
    ...
    function changePricingID($order_id,$new_pricing_id,$create_invoice=false) {
    ...
    function renew($order_id, $date=null) {
        if(is_null($date)) {
            $date = 'next_due_date';
        } else {
            $date = "'".$date."'";
        }
        $this->db->Execute("UPDATE ".T_ORDERS." SET next_due_date = IF(future_due_date='0000-00-00',IF(period='days',$date+INTERVAL period_count DAY,IF(period='months',$date+INTERVAL period_count MONTH,$date+INTERVAL period_count YEAR)),future_due_date) WHERE id=?",array($order_id));
    ...
    function changeStatus($order_id,$new_status) {
        $order = $this->get($order_id);
        ...
        if($new_status == 'active') {
            ...
            $order_invoices = $this->db->GetAll("SELECT id FROM ".T_INVOICES." WHERE status='unpaid'");
    ...
    function changeProductStatus($order_id,$new_status) {
        $order = $this->get($order_id);
