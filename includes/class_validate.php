...
class Validate_Banned_Words {
    ...
    */
    function validate($value) {
        $words = preg_split('/[^\w]+/',$this->PMDR->getConfig('banned_words'),null,PREG_SPLIT_NO_EMPTY);
        if(count($words)) {
            $words = implode('|',array_map('preg_quote',$words));
            $value = strip_tags($value);
            $matches = array();
            if(preg_match('/\b('.$words.')\b/i', $value, $matches)) {
    ...
    */
    function validate($value) {
        ...
        foreach($values AS $value) {
            if($value != '' AND !preg_match("/^[A-Za-z0-9._%\-+]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,6}$/", $value)) {
                $this->error = $this->PMDR->getLanguage('messages_email_format');
                return false;
            }
        }
...
class Validate_Image {
    ...
    */
    function __construct($width, $height, $filesize, $formats, $allow_animated = false, $force_dimensions = false) {
        $this->PMDR = Registry::getInstance();
        $this->width = $width;
        $this->height = $height;
    ...
    */
    function validate($value) {
        // If image size is zero we do not have an image so it is validated
        if($value['size'] == 0) {
            return true;
        }
        // Check if we have a valid image
        if($image_details = get_image_size($value['tmp_name'])) {
            // If animated is not allowed, but image is animated, return false
            if($this->allow_animated == false AND is_animated_gif($value['tmp_name'])) {
                $this->error = $this->PMDR->getLanguage('messages_images_animation_error');
                return false;
            // If the image dimensions are not valid, return false
            } elseif(($image_details[0] > $this->width OR $image_details[1] > $this->height) AND ($this->force_dimensions OR ($this->allow_animated AND is_animated_gif($value['tmp_name'])))) {
            ...
            } elseif(filesize($value['tmp_name'])/1024 > $this->filesize) {
            ...
            } elseif(!in_array(strtolower($image_details[2]),$this->formats)) {
...
class Validate_Price {
    ...
    */
    function validate($value) {
        if(preg_match('/^[0-9'.$this->PMDR->getLanguage('thousandseperator').$this->PMDR->getLanguage('thousandseperator').$this->PMDR->getLanguage('decimalseperator').$this->PMDR->getLanguage('currency_prefix').$this->PMDR->getLanguage('currency_suffix').'\.]+$/',$value)) {
            return true;
        } else {
