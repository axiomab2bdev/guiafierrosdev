...
foreach($listings_results as $key=>$listing) {
    ...
    $template_content_results->set('description', Strings::limit_characters($listing['description'],$listing['description_size']));

    $description = Strings::limit_characters($listing['description'],$listing['description_size']);
    ...
    if($listing['short_description_size']) {
        if(!empty($listing['description_short'])) {
            $template_content_results->set('short_description', nl2br(Strings::limit_characters($listing['description_short'],$listing['short_description_size'])));
        } elseif($listing['description_size'] AND !empty($listing['description'])) {
            $template_content_results->set('short_description', nl2br(Strings::limit_characters(strip_tags($description,'<br><br/>'),$listing['short_description_size'])));
        }
    } else {
    ...
    if($PMDR->get('Listings')->ifNew($listing['date'])) { $template_content_results->set('new', '<span class="listing_new">'.$PMDR->getLanguage('public_listing_new').'</span>'); }
    if($PMDR->get('Listings')->ifUpdated($listing['date_update'])) { $template_content_results->set('updated', '<span class="listing_updated">'.$PMDR->getLanguage('public_listing_updated').'</span>'); }
    if($PMDR->get('Listings')->ifHot($listing['rating'])) { $template_content_results->set('hot','<span class="listing_hot">'.$PMDR->getLanguage('public_listing_hot').'</span>'); }
    ...
    } elseif(ADDON_WEBSITE_SCREENSHOTS AND $listing['www_screenshot_allow'] AND file_exists(SCREENSHOTS_PATH.$listing['id'].'-small.jpg')) {
        $template_content_results->set('logo_url',get_file_url_cdn(SCREENSHOTS_PATH.$listing['id'].'-small.jpg'));
    } elseif($PMDR->get('Templates')->path('images/noimage.gif')) {
        $template_content_results->set('logo_url',$PMDR->get('Templates')->urlCDN('images/noimage.gif'));
    }
