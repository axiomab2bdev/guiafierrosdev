...
function cron_sitemaps($j) {
    ...
    // Try to ping Yahoo.com and record results if successful
    if($sitemap->pingSearchEngine('http://search.yahooapis.com/SiteExplorerService/V1/ping?sitemap=',BASE_URL.'/xml.php?type=sitemap')) {
        $cron_data['data']['sitemaps_submitted'][] = 'yahoo';
        $cron_data['status'] = true;
    }
