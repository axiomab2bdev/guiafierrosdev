...
function cron_contact_requests($j) {
    ...
    foreach($requests AS $request) {
        ...
        if($PMDR->getConfig('contact_requests_messages')) {
            ...
            $db->Execute("INSERT INTO ".T_MESSAGES_POSTS." (message_id,user_id,content,date_sent) SELECT id, user_id_from, '".$request['message']."', NOW() FROM ".T_MESSAGES." WHERE contact_request_id=?",array($request['id']));
        } else {
