...
function cron_invoices($j) {
    ...
    // Because the next invoice date will be 0000-00-00 if its a free product we don't need to check its price for invoicing
    $orders = $db->GetAll("
        SELECT
            o.id,
            o.user_id,
        ...
        WHERE
            ...
            AND next_invoice_date != '0000-00-00'"
    );
    $cron_invoices = array();
    foreach($orders AS $order) {
        ...
        if($order['discount_code_type'] != 'onetime') {
        ...
        // An option can be added for non-continuous integration and if turned on next_invoice_date can be set to 0000-00-00 until all
