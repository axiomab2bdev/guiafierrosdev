...
switch($_POST['action']) {
    ...
    case 'admin_listings_csv_statistics':

        $PMDR->loadLanguage(array('admin_listings_csv'));

        $PMDR->get('Authentication')->checkPermission('admin_statistics');

        if(((int) $_POST['start']) == 0) {

            @unlink(TEMP_UPLOAD_PATH.'listing_statistics.csv');



            $header = array(

                $PMDR->getLanguage('admin_listings_csv_listing_id'),

                $PMDR->getLanguage('admin_listings_csv_listing_title'),

                $PMDR->getLanguage('admin_listings_csv_visits'),

                $PMDR->getLanguage('admin_listings_csv_messages'),

                $PMDR->getLanguage('admin_listings_csv_website'),

                $PMDR->getLanguage('admin_listings_csv_banner_exposures'),

                $PMDR->getLanguage('admin_listings_csv_banner_clicks')
            ...
            $handle = fopen(TEMP_UPLOAD_PATH.'listing_statistics.csv', 'w');
        ...
        $listings = $db->GetAll("SELECT id, title, impressions, emails, website_clicks, banner_impressions, banner_clicks FROM ".T_LISTINGS." ORDER BY id ASC LIMIT ?,?",array((int) $_POST['start'],(int) $_POST['num']));

        $handle = fopen(TEMP_UPLOAD_PATH.'listing_statistics.csv', 'a');
        ...
        foreach($listings AS $l) {
            ...
            $csv_output .= "\"$l[id]\",\"$l[title]\",\"$l[impressions]\",\"$l[emails]\",\"$l[website_clicks]\",\"$l[banner_impressions]\",\"$l[banner_clicks]\"\r\n";
    ...
    case 'admin_maintenance_duplicates':
        ...
        switch($_POST['type']) {
            ...
            case 'user_ip':
                ...
                $result['results'] = $db->GetAll("SELECT id, logged_ip, COUNT(id) AS count FROM ".T_USERS." WHERE logged_ip != '' GROUP BY logged_ip HAVING count > 1");

                break;

            case 'user_name':

                $result['results'] = $db->GetAll("SELECT id, user_first_name, user_last_name, CONCAT(user_first_name,user_last_name) AS name, COUNT(id) AS count FROM ".T_USERS." GROUP BY name HAVING name!='' AND count > 1");

                break;

            case 'user_phone':

                $result['results'] = $db->GetAll("SELECT id, user_phone, COUNT(id) AS count FROM ".T_USERS." GROUP BY user_phone HAVING user_phone!='' AND count > 1");

                break;

            case 'listing_title':

                $result['results'] = $db->GetAll("SELECT id, title, COUNT(id) AS count FROM ".T_LISTINGS." GROUP BY title HAVING count > 1");

                break;

            case 'listing_www':

                $result['results'] = $db->GetAll("SELECT id, www, COUNT(id) AS count FROM ".T_LISTINGS." GROUP BY www HAVING www!='' AND count > 1");

                break;

            case 'listing_phone':

                $result['results'] = $db->GetAll("SELECT id, phone, COUNT(id) AS count FROM ".T_LISTINGS." GROUP BY phone HAVING phone!='' AND count > 1");
    ...
    case 'admin_categories_sort':
        ...
        if(((int) $_POST['start']) == 0) {
            ...
            $create_table = $db->GetRow("SHOW CREATE TABLE ".T_CATEGORIES);

            // Rename current table to a tmp table

            $db->Execute("DROP TABLE IF EXISTS ".T_CATEGORIES."_tmp");

            $db->Execute("RENAME TABLE ".T_CATEGORIES." TO ".T_CATEGORIES."_tmp");
        ...
        $records = $db->GetAll("SELECT * FROM ".T_CATEGORIES."_tmp ORDER BY level, title LIMIT ?,?",array((int) $_POST['start'],(int) $_POST['num']));
        ...
        foreach($records as $record) {
            ...
            if($record['level'] == '0') {
                ...
                $db->Execute("INSERT INTO ".T_CATEGORIES." SET id=?, title=?, level=?, left_=?, right_=?, parent_id=?, impressions=?",array(1,'ROOT',0,0,1,NULL,0));

            } else {

                // Get the parent of the current record so we know where to insert it

                $record['placement_id'] = $db->GetOne("SELECT id FROM ".T_CATEGORIES."_tmp WHERE left_ < ".$record['left_']." AND right_ > ".$record['right_']." ORDER BY left_ DESC");

                $record['placement'] = 'subcategory';

                // Insert the category

                $PMDR->get('Categories')->insert($record);
        ...
        $return = array(
            ...
            'percent'=>floor(($count*100) / $db->GetOne("SELECT COUNT(*) FROM ".T_CATEGORIES."_tmp")),
        ...
        if($return['percent'] == 100) {
            ...
            if($db->GetOne("SELECT COUNT(*) FROM ".T_CATEGORIES) == $db->GetOne("SELECT COUNT(*) FROM ".T_CATEGORIES."_tmp"))  {

                // Delete original table

                $db->Execute("DROP TABLE ".T_CATEGORIES."_tmp");
                ...
                $db->Execute("DROP TABLE ".T_CATEGORIES);

                $db->Execute("RENAME TABLE ".T_CATEGORIES."_tmp  TO ".T_CATEGORIES);
    ...
    case 'admin_locations_sort':
        ...
        foreach($records as $record) {
            ...
            if($record['level'] == '0') {
                ...
                $record['placement_id'] = $db->GetOne("SELECT id FROM ".T_LOCATIONS."_tmp WHERE left_ < ".$record['left_']." AND right_ > ".$record['right_']." ORDER BY left_ DESC");
    ...
    case 'admin_locations_expand':
        ...
        foreach($records AS $record) {
            ...
            <tr class="'.$parent['id'].'_child">
                ...
                $data .= '<td><a href="admin_listings.php?'.$query_string_name.'='.$record['id'].'">'.$record['count_total'].'</a></td>';
    ...
    case 'admin_categories_search':
        ...
        $records = $db->GetAll("SELECT id, title FROM ".T_CATEGORIES." WHERE title LIKE ".$db->Clean($_POST['value']."%")." ORDER BY title LIMIT 20");
        ...
        if(count($records)) {
            ...
            foreach($records AS $record) {
                ...
                $record_path = $PMDR->get('Categories')->getPath($record['id']);
                ...
                foreach($record_path AS $key=>$path) {
                    ...
                    $data .= '<a href="'.BASE_URL_ADMIN.'/admin_categories.php?action=edit&id='.$path['id'].'">';
    ...
    case 'admin_quick_search':
        ...
        if(!is_numeric($_POST['value'])) {
            ...
            if($listings = $db->GetAll("SELECT id, user_id, title FROM ".T_LISTINGS." WHERE MATCH(title) AGAINST (".$db->Clean($_POST['value'].'*')." IN BOOLEAN MODE) ORDER BY title LIMIT 10")) {
            ...
            if($settings = $db->GetAll("SELECT s.*, l.content FROM ".T_LANGUAGE_PHRASES." l INNER JOIN ".T_SETTINGS." s ON l.variablename = CONCAT('setting_',s.varname,'_title') WHERE l.section='settings' AND (l.content LIKE ".$db->Clean($_POST['value']."%")." OR s.varname LIKE ".$db->Clean($_POST['value']."%").")")) {
                ...
                foreach($settings AS $key=>$setting) {
                    ...
                    if($setting['group'] == 'website_screenshots' AND !ADDON_WEBSITE_SCREENSHOTS) {
                    ...
                    if($setting['group'] == 'blog' AND !ADDON_BLOG) {
        ...
        if($users OR $listings OR $invoice OR $order OR $settings) {
    ...
    case 'admin_export':
        ...
        if($export) {
            ...
            $listings = $db->GetAll("SELECT SQL_CALC_FOUND_ROWS l.*, o.pricing_id FROM ".T_LISTINGS." l INNER JOIN ".T_ORDERS." o ON l.id=o.type_id AND o.type='listing_membership' $category_sql ORDER BY l.id LIMIT ?,?",array((int) $_POST['start'],(int) $_POST['num']));
    ...
    case 'admin_check_table':
        ...
        // Delay this a bit to prevent forbidden errors
    ...
    case 'admin_categories_export':
        ...
        if(((int) $_POST['start']) == 0) {
            ...
            @unlink(TEMP_UPLOAD_PATH.'categories_export.csv');

            $category_labels = $PMDR->get('Categories')->getLevelLabels();

            $csv_output = "\"".implode('","',$category_labels)."\"\r\n";

            $handle = fopen(TEMP_UPLOAD_PATH.'categories_export.csv', 'w');
        ...
        $records = $db->GetAll("SELECT * FROM ".T_CATEGORIES." WHERE id!=1 ORDER BY left_ ASC LIMIT ?,?",array((int) $_POST['start'],(int) $_POST['num']));

        $handle = fopen(TEMP_UPLOAD_PATH.'categories_export.csv', 'a');
        ...
        foreach($records AS $record) {
            ...
            $category_path = $PMDR->get('Categories')->getPath($record['id']);
        ...
        $return = array(
            ...
            'percent'=>(!$records ? 100 : floor(($count*100) / ($db->GetOne("SELECT COUNT(*) FROM ".T_CATEGORIES) - 1))),
