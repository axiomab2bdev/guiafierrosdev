...
if($_GET['action'] == 'approve') {
    ...
    if(!isset($_GET['confirmed']) AND $db->GetOne("SELECT COUNT(*) FROM ".T_LISTINGS_CLAIMS." WHERE listing_id=? AND id!=?",array($_GET['listing_id'],$_GET['id']))) {
        ...
        $db->Execute("UPDATE ".T_LISTINGS." SET claimed=1 WHERE id=?",array($_GET['listing_id']));

        $db->Execute("DELETE FROM ".T_LISTINGS_CLAIMS." WHERE listing_id=?",array($_GET['id']));

        $listings->changeUser($_GET['listing_id'],$_GET['user_id']);
