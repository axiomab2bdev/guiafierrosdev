...
if(!isset($_GET['action'])) {
    ...
    $form->addFieldSet('categories',array('legend'=>$PMDR->getLanguage('admin_export_category_list')));
    ...
    $form->addField('category_list','tree_select_multiple',array('label'=>$PMDR->getLanguage('admin_export_category_list'),'fieldset'=>'categories','value'=>'','options'=>$db->GetAssoc("SELECT id, title, level FROM ".T_CATEGORIES." WHERE id!=1 ORDER BY left_")));
    ...
    if($form->wasSubmitted('export_submit')) {
        ...
        if(!$form->validate()) {
            ...
            $db->Execute("INSERT INTO ".T_EXPORTS." (amount,format,delimiter,categories,data) VALUES (?,?,?,?,?)",array($data['limit'],$data['format'],$delimiter,implode(',',$data['category_list']),implode(',',$data['fields'])));
