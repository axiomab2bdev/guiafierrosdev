...
$PMDR->loadLanguage(array('admin_ratings','admin_listings','admin_reviews','admin_reviews_comments','admin_users'));
...
if(!isset($_GET['action'])) {
    ...
    foreach($records as $key=>$record) {
        ...
        if(!is_null($record['review_id']) AND $record['review_status'] != 'active') {
            ...
            $records[$key]['review_id'] .= ' <i id="review_status_icon'.$key.'" class="icon-exclamation-sign"></i>';
    ...
    $form->addField('rating','stars',array('label'=>$PMDR->getLanguage('admin_ratings_rating'),'fieldset'=>'information','value'=>0));
