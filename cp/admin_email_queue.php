...
if($_GET['action'] == 'approve') {
    ...
    $db->Execute("UPDATE ".T_EMAIL_QUEUE." SET moderate=0 WHERE id=?",array($_GET['id']));
...
if($_GET['action'] == 'process') {
    ...
    $PMDR->get('Email_Queue')->processEmail($_GET['id']);

    $PMDR->addMessage('success',$PMDR->getLanguage('admin_email_queue_processed'));
...
if($_GET['action'] == 'delete') {
    ...
    $db->Execute("DELETE FROM ".T_EMAIL_QUEUE." WHERE id=?",array($_GET['id']));
...
if(isset($_POST['table_list_submit'])) {
    ...
    if($_POST['action'] == 'delete') {
        ...
        foreach($_POST['table_list_checkboxes'] AS $id) {
            ...
            $db->Execute("DELETE FROM ".T_EMAIL_QUEUE." WHERE id=?",array($id));
    ...
    if($_POST['action'] == 'approve') {
        ...
        foreach($_POST['table_list_checkboxes'] AS $id) {
            ...
            $db->Execute("UPDATE ".T_EMAIL_QUEUE." SET moderate=0 WHERE id=?",array($id));
...
foreach($records as $key=>$record) {
    ...
    if($record['moderate']) {
        ...
        $records[$key]['campaign_id'] .= ' <i class="icon-flag"></i>';
