...
if(!isset($_GET['action'])) {
    ...
    if($category_count > 1) {
        ...
        if($PMDR->getConfig('category_select_type') == 'tree_select') {
            ...
            $form->addField('categories','tree_select',array('value'=>$_GET['category'],'first_option'=>'','options'=>$db->GetAssoc("SELECT id, title, level FROM ".T_CATEGORIES." WHERE ID != 1 AND hidden=0 ORDER BY left_")));
        ...
        $form->addField('categories','hidden',array('value'=>$db->GetOne("SELECT id FROM ".T_CATEGORIES." WHERE id!=1")));
