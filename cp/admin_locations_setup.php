...
$PMDR->getNew('PMDLicense')->validate();
...
if($form->wasSubmitted('submit') OR $form->wasSubmitted('submit')) {
    ...
    if(!$form->validate()) {
        ...
        $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='Location Text 1' WHERE variablename='general_locations_text_1' AND languageid != -1");

        $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='Location Text 2' WHERE variablename='general_locations_text_2' AND languageid != -1");

        $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='Location Text 3' WHERE variablename='general_locations_text_3' AND languageid != -1");
        ...
        if($data['country'] == 'static') {
            ...
            if($data['country_type'] == 'type_in') {
                ...
                $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='Country' WHERE variablename='general_locations_text_".$location_text_field."' AND languageid != -1 AND content='Location Text ".$location_text_field."'");
                ...
                $messages[] = 'Users will select their country from a list.  The country "<a href="admin_locations.php?action=edit&id='.$placement_id.'">'.$data['country_option'].'</a>" has been added to the list.  You may also <a href="admin_locations.php?action=add">add additional countries.';
        ...
        if($data['state'] == 'static') {
            ...
            if($data['state_type'] == 'type_in') {
                ...
                $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='State' WHERE variablename='general_locations_text_".$location_text_field."' AND languageid != -1 AND content='Location Text ".$location_text_field."'");
        ...
        if($data['city'] == 'static') {
            ...
            if($data['city_type'] == 'type_in') {
                ...
                $db->Execute("UPDATE ".T_LANGUAGE_PHRASES." SET content='City' WHERE variablename='general_locations_text_".$location_text_field."' AND languageid != -1 AND content='Location Text ".$location_text_field."'");
