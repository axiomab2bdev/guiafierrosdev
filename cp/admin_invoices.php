...
if(!isset($_GET['action']) OR $_GET['action'] == 'search') {
    ...
    foreach($records as $key=>$record) {
        ...
        $records[$key]['status'] = '<span class="label '.($record['status'] == 'paid' ? 'label-success' : ($record['status'] == 'unpaid' ? 'label-warning' : '')).'">'.$PMDR->getLanguage('admin_invoices_'.$record['status']).'</span>';
