...
if(!isset($_GET['action']) OR $_GET['action'] == 'search') {
    ...
    SELECT v.*, u.user_first_name, u.user_last_name, u.login, p.name, pp.label, l.title
    ...
    foreach($records as $key=>$record) {
        ...
        if(!$records[$key]['next_due_date'] = $PMDR->get('Dates_Local')->formatDate($record['next_due_date'])) {
            ...
            $records[$key]['next_due_date'] = '<span class="text-error">'.$records[$key]['next_due_date'].'</span>';

            if($db->GetOne("SELECT COUNT(*) FROM ".T_INVOICES." WHERE order_id=? AND status='unpaid'",array($record['id']))) {

                $records[$key]['next_due_date'] .= ' <a href="admin_invoices.php?order_id='.$record['id'].'"><i title="Unpaid Invoices" class="icon-warning-sign"><i></a>';
        ...
        if(!is_null($record['title'])) {
            ...
            $records[$key]['product'] .= '<br /><a href="'.BASE_URL_ADMIN.'/admin_listings.php?action=edit&id='.$record['type_id'].'&user_id='.$record['user_id'].'">'.$PMDR->get('Cleaner')->clean_output($record['title']).'</a>';
    ...
    if($invoices = $db->GetAll("SELECT * FROM ".T_INVOICES." WHERE order_id=? ORDER BY date",array($order['id']))) {

        $order['invoices'] = '';

        foreach($invoices as $invoice) {

            $order['invoices'] .= '<a href="admin_invoices.php?action=edit&id='.$invoice['id'].'&user_id='.$invoice['user_id'].'"><span style="color: '.($invoice['status'] == 'paid' ? 'green' : 'red').';">'.$invoice['id'].'</span></a>, ';

        }

        $order['invoices'] = rtrim($order['invoices'],' ,');
    ...
    $order['invoices'] .= ' <a class="btn btn-mini" href="'.BASE_URL_ADMIN.'/admin_invoices.php?order_id='.$order['id'].'&user_id='.$order['user_id'].'">View All</a>';

    $order['invoices'] .= ' <a class="btn btn-mini" href="'.BASE_URL_ADMIN.'/admin_invoices.php?action=add&order_id='.$order['id'].'&user_id='.$order['user_id'].'">Add Invoice</a>';
    ...
    $form->addFieldSet('order',array('legend'=>'Order Details'));

    $form->addFieldSet('order_status',array('legend'=>'Order Status'));

    if(!$PMDR->getConfig('disable_billing')) {

        $form->addFieldSet('order_modify',array('legend'=>'Modify Order'));

    }

    $form->addField('id','custom',array('label'=>$PMDR->getLanguage('admin_orders_id'),'fieldset'=>'order'));

    $form->addField('order_id','custom',array('label'=>$PMDR->getLanguage('admin_orders_number'),'fieldset'=>'order'));

    $form->addField('date','custom',array('label'=>$PMDR->getLanguage('admin_orders_date'),'fieldset'=>'order','html'=>$PMDR->get('Dates_Local')->formatDate($order['date'])));
    ...
    if(!$PMDR->getConfig('disable_billing')) {
        ...
        $form->addField('status','select',array('label'=>$PMDR->getLanguage('admin_orders_status'),'fieldset'=>'order_status','options'=>$status_options));

        $form->addField('notify_user','checkbox',array('label'=>$PMDR->getLanguage('admin_orders_notify_user'),'fieldset'=>'order_status','value'=>''));

    } else {

        $form->addField('status','custom',array('label'=>$PMDR->getLanguage('admin_orders_status'),'fieldset'=>'order_status','html'=>$status_options[$order['status']]));

        $form->addField('product_status','custom',array('label'=>'Product Status','fieldset'=>'order_status','html'=>$status_options[$order['product_status']]));

    }

    $form->addField('product','custom',array('label'=>$PMDR->getLanguage('admin_orders_product'),'fieldset'=>'order','html'=>$order['product_type'].' (<a href="admin_products.php?action=edit&id='.$order['product_id'].'">'.$order['product_name'].'</a>) - <a href="admin_listings.php?action=edit&id='.$order['type_id'].'&user_id='.$order['user_id'].'">'.$order['product_title'].'</a>'));

    if($PMDR->getConfig('user_select') == 'select_window') {

        $form->addField('user_id','select_window',array('label'=>$PMDR->getLanguage('admin_orders_user'),'fieldset'=>'order','icon'=>'users_search','options'=>'select_user'));

    } else {

        $form->addField('user_id','select',array('label'=>$PMDR->getLanguage('admin_orders_user'),'fieldset'=>'order','options'=>$db->GetAssoc("SELECT id, CONCAT(login, ' (',user_email,')') FROM ".T_USERS." ORDER BY login")));

    }

    if(!$PMDR->getConfig('disable_billing')) {

        $form->addField('invoices','custom',array('label'=>'Invoices','fieldset'=>'order','html'=>$order['invoices']));

    }

    $form->addField('ip_address','custom',array('label'=>$PMDR->getLanguage('admin_orders_ip_address'),'fieldset'=>'order','html'=>$order['ip_address']));
    ...
    if(!$PMDR->getConfig('disable_billing')) {
        ...
        $discount_codes = $db->GetAssoc("SELECT code, CONCAT(title,' (',code,')') AS value FROM ".T_DISCOUNT_CODES." ORDER BY title ASC");

        $form->addField('discount','select',array('label'=>'Discount','first_option'=>'','fieldset'=>'order','value'=>$order['discount_code'],'options'=>$discount_codes));

        $form->addFieldNote('discount','Changing the discount code will not automatically update the pricing.');
    ...
    if($form->wasSubmitted('submit')) {
        ...
        if(!$form->validate()) {
            ...
            if($order['pricing_id'] != $data['pricing_id']) {
            ...
            if($data['product_status'] != $order['product_status']) {
