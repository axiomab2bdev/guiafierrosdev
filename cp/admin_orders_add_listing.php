...
if(!isset($_GET['primary_category_id'])) {
    ...
    if($category_count > 1) {
        ...
        if($PMDR->getConfig('category_select_type') == 'tree_select') {
            ...
            $form->addField('primary_category_id','tree_select',array('label'=>$PMDR->getLanguage('admin_listings_primary_category'),'fieldset'=>'listing','first_option'=>'','options'=>$db->GetAssoc("SELECT id, title, level FROM ".T_CATEGORIES." WHERE ID != 1 ORDER BY left_")));
        ...
        redirect(array('user_id'=>$_GET['user_id'],'create_invoice'=>$_GET['create_invoice'],'pricing_id'=>$product['pricing_id'],'primary_category_id'=>$db->GetOne("SELECT id FROM ".T_CATEGORIES." WHERE id!=1")));
    ...
    if($category_count > 1 AND $product['category_limit'] > 1) {
        ...
        if($PMDR->getConfig('category_select_type') == 'tree_select' OR $PMDR->getConfig('category_select_type') == 'tree_select_multiple') {
            ...
            if($product['category_limit'] == 2) {
                ...
                $form->addField('categories','tree_select_multiple',array('label'=>$PMDR->getLanguage('admin_listings_categories'),'fieldset'=>'listing','limit'=>$product['category_limit'],'options'=>$db->GetAssoc("SELECT id, title, level FROM ".T_CATEGORIES." WHERE ID != 1 ORDER BY left_")));
            ...
            if($product['category_limit'] == 2) {
                ...
                $form->addField('categories','tree_select_cascading',array('label'=>$PMDR->getLanguage('admin_listings_categories'),'fieldset'=>'listing','options'=>array('type'=>'category_tree','limit'=>$product['category_limit'])));

            } else {

                $form->addField('categories','tree_select_cascading_multiple',array('label'=>$PMDR->getLanguage('admin_listings_categories'),'fieldset'=>'listing','options'=>array('type'=>'category_tree','limit'=>$product['category_limit'])));
            ...
            if($product['category_limit'] == 2) {
                ...
                $form->addField('categories','tree_select_expanding_radio',array('label'=>$PMDR->getLanguage('admin_listings_categories'),'fieldset'=>'listing','options'=>array('type'=>'category_tree')));

            } else {

                $form->addField('categories','tree_select_expanding_checkbox',array('label'=>$PMDR->getLanguage('admin_listings_categories'),'fieldset'=>'listing','limit'=>$product['category_limit'],'options'=>array('type'=>'category_tree')));
    ...
    if($product['hours_allow']) {
        ...
        $form->addField('hours','hours',array('label'=>$PMDR->getLanguage('admin_listings_hours'),'fieldset'=>'listing'));
