    ...
    if(!$import_running) {
        ...
        $imports->update(
            ...
            array(
                ...
                'date_activity'=>'0000-00-00 00:00:00',
...
if(!isset($_GET['action']) OR $_GET['action'] == 'search') {
    ...
    foreach($records as $key=>$record) {
        ...
        if($record['status'] == 'running') {
            ...
            $records[$key]['status'] .= ' <a class="btn btn-mini btn-warning" href="admin_imports.php?action=pause&id='.$record['id'].'">'.$PMDR->getLanguage('admin_imports_pause').'</a>';
        ...
        if($record['status'] == 'paused') {
            ...
            $records[$key]['status'] .= ' <a class="btn btn-mini btn-success" href="admin_imports.php?action=resume&id='.$record['id'].'">'.$PMDR->getLanguage('admin_imports_resume').'</a>';

        }

        if($record['status'] == 'pending' AND $record['scheduled']) {

            $records[$key]['status'] .= ' <i class="icon-time" title="Scheduled"></i>';

        }

        if($record['notifications']) {

            $records[$key]['name'] .= ' <i class="icon-envelope" title="Notifications On"></i>';
