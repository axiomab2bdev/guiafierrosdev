...
$PMDR->loadLanguage(array('admin_reviews','admin_listings','admin_ratings','admin_reviews_comments','admin_users'));
...
if($_GET['action'] == 'approve') {
    ...
    $PMDR->addMessage('success',$PMDR->getLanguage('admin_reviews_approved',$_GET['id']),'delete');
...
if(!isset($_GET['action'])) {
    ...
    SELECT reviews.*, u.user_first_name, u.user_last_name, u.login, ra.rating, COUNT(rc.id) AS comments
    ...
    foreach($records as $key=>$record) {
        ...
        $records[$key]['rating'] = $PMDR->get('Ratings')->printRatingStatic($record['rating']);
    ...
    $form->addField('rating','stars',array('label'=>$PMDR->getLanguage('admin_reviews_rating'),'fieldset'=>'information','value'=>0));
