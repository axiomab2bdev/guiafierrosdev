...
if(!isset($_GET['action'])) {
    ...
    if($_GET['keyword'] != '' AND !empty($_GET['filter'])) {
        ...
        if($_GET['keyword_options'] != 'EXACT') {
            ...
            $keywords = preg_split("/[\s]+/", $_GET['keyword']);
    ...
    $table_list->addColumn('master_phraseid',$PMDR->getLanguage('admin_languages_id'));
    ...
    $table_list->addColumn('content',$PMDR->getLanguage('admin_languages_content'));
    ...
    foreach($records as $key=>$record) {
        ...
        $form_edit->addField('master_phrase_'.$record['master_section'].'_'.$record['master_variablename'],'textarea',array('label'=>$PMDR->getLanguage('admin_languages_content'),'fieldset'=>'phrase','value'=>$record['master_content'],'readonly'=>true,'style'=>'height: 40px'));

        $form_edit->addField('phrase-'.$record['master_section'].'-'.$record['master_variablename'],'textarea',array('label'=>$PMDR->getLanguage('admin_languages_content'),'fieldset'=>'phrase','value'=>$record['content'],'style'=>'height: 40px'));



        $records[$key]['content'] = $form_edit->getFieldHTML('master_phrase_'.$record['master_section'].'_'.$record['master_variablename']);

        $records[$key]['content'] .= '<br /><br />';
