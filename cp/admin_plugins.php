...
if($_GET['action'] == 'enable') {
    ...
    $db->Execute("UPDATE ".T_PLUGINS." SET active=1 WHERE id=?",array($_GET['id']));
...
if($_GET['action'] == 'disable') {
    ...
    $db->Execute("UPDATE ".T_PLUGINS." SET active=0 WHERE id=?",array($_GET['id']));
...
if($_GET['action'] == 'install') {
    ...
    $plugin = $db->GetRow("SELECT * FROM ".T_PLUGINS." WHERE id=?",array($_GET['id']));

    $db->Execute("UPDATE ".T_PLUGINS." SET installed=1, active=1 WHERE id=?",array($plugin['id']));
    ...
    if(file_exists(PLUGINS_PATH.$plugin['id'].'/'.$plugin['id'].'_install.php')) {
        ...
        if(!@eval('return true;?>'.file_get_contents(PMDROOT.'/modules/plugins/'.$plugin['id'].'/'.$plugin['id'].'_install.php'))) {
            ...
            $db->Execute("UPDATE ".T_PLUGINS." SET installed=0 WHERE id=?",array($plugin['id']));
...
if($_GET['action'] == 'uninstall') {
    ...
    $PMDR->get('Authentication')->checkPermission('admin_plugins_delete');

    $plugin = $db->GetRow("SELECT * FROM ".T_PLUGINS." WHERE id=?",array($_GET['id']));

    $db->Execute("UPDATE ".T_PLUGINS." SET installed=0, active=0 WHERE id=?",array($plugin['id']));
    ...
    if(file_exists(PLUGINS_PATH.$plugin['id'].'/'.$plugin['id'].'_uninstall.php')) {
        ...
        if(!@eval('return true;?>'.file_get_contents(PMDROOT.'/modules/plugins/'.$plugin['id'].'/'.$plugin['id'].'_uninstall.php'))) {
            ...
            $db->Execute("UPDATE ".T_PLUGINS." SET installed=1 WHERE id=?",array($plugin['id']));
