...
if(!isset($_GET['action']) OR $_GET['action'] == 'search') {
    ...
    foreach($records as $key=>$record) {
        ...
        $records[$key]['manage'] .= $PMDR->get('HTML')->icon('login_lock_arrow',array('target'=>'_blank','href'=>BASE_URL.MEMBERS_FOLDER.'index.php?user_login='.$PMDR->get('Cleaner')->clean_output($record['login']),'label'=>$PMDR->getLanguage('admin_users_login')));
    ...
    $form->addField('profile_image','file',array('fieldset'=>'user_details'));
    ...
    if($_GET['action'] == 'edit') {
        ...
        if(!$PMDR->get('Authentication')->checkPermission('admin_users_groups_edit',false)) {
            ...
            $form->setFieldAttribute('user_groups','options',$db->GetAssoc("SELECT g.id, g.name FROM ".T_USERS_GROUPS." g WHERE g.id NOT IN(SELECT group_id FROM ".T_USERS_GROUPS_PERMISSIONS_LOOKUP." gl WHERE gl.permission_id=71)"));
