...
if(!isset($_GET['action'])) {
    ...
    foreach($records as &$record) {
        ...
        if(file_exists(IMAGES_THUMBNAILS_PATH.$record['id'].'.'.$record['extension'])) {
            ...
            $record['image'] = '<a href="'.get_file_url(IMAGES_PATH.$record['id'].'.'.$record['extension'],true).'" title="'.$record['title'].' - '.$record['description'].'" class="image_group" rel="image_group"><img border="0" src="'.get_file_url(IMAGES_THUMBNAILS_PATH.$record['id'].'.'.$record['extension'],true).'"></a>';
    ...
    $form->addField('image','file',array('label'=>$PMDR->getLanguage('admin_images_image'),'fieldset'=>'image_details'));
    ...
    $form->addValidator('image',new Validate_Image($PMDR->getConfig('gallery_image_width'),$PMDR->getConfig('gallery_image_height'),$PMDR->getConfig('gallery_image_size'),explode(',',$PMDR->getConfig('images_formats'))));
    ...
    if($_GET['action'] == 'edit') {
        ...
        if(file_exists(IMAGES_THUMBNAILS_PATH.$edit_image['id'].'.'.$edit_image['extension'])) {
            ...
            $form->addField('preview','custom',array('label'=>$PMDR->getLanguage('admin_images_current'),'fieldset'=>'image_details','value'=>'','options'=>'','html'=>'<a href="'.get_file_url(IMAGES_PATH.$edit_image['id'].'.'.$edit_image['extension'],true).'" class="image_group" title="'.$edit_image['title'].'"><img border="0" src="'.get_file_url(IMAGES_THUMBNAILS_PATH.$edit_image['id'].'.'.$edit_image['extension'],true).'"></a>'));
