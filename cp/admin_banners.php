...
if(!isset($_GET['action'])) {
    ...
    $form->addField('code_text','text',array('fieldset'=>'banner'));
    ...
    $form->addField('categories','tree_select_expanding_checkbox',array('fieldset'=>'banner','options'=>array('type'=>'category_tree','bypass_setup'=>true)));

    $form->addField('locations','tree_select_expanding_checkbox',array('fieldset'=>'banner','options'=>array('type'=>'location_tree','bypass_setup'=>true)));
    ...
    if($_GET['action'] == 'edit') {
        ...
        if($edit_banner['extension'] == 'swf') {
            ...
            $preview = '<a href="'.get_file_url(BANNERS_PATH.$edit_banner['id'].'.'.$edit_banner['extension'],true).'" class="image_group" title="'.$edit_banner['title'].'"><img border="0" src="'.get_file_url(BANNERS_PATH.$edit_banner['id'].'.'.$edit_banner['extension'],true).'"></a>';
        ...
        $form->addValidator('image',new Validate_NonEmpty_File());
    ...
    if($form->wasSubmitted('submit')) {
        ...
        $type = $db->GetRow("SELECT type, width, height, filesize FROM ".T_BANNER_TYPES." WHERE id=?",array($data['type_id']));



        if($type['type'] == 'image') {

            $form->addValidator('image',new Validate_Image($type['width'],$type['height'],$type['filesize'],$PMDR->getConfig('banners_formats'),true));
