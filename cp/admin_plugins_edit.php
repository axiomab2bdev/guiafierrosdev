...
if(in_array(get_file_format(PMDROOT.'/modules/plugins/'.$_GET['id'].'/'.$_GET['file']),array('image/gif','image/jpg','image/png','image/jpeg'))) {
    ...
    $form->addField('code','textarea',array('label'=>'','value'=>$code,'fieldset'=>'plugin_details','style'=>'font-family: Courier New; width: 100%; height: 400px'));
...
foreach($files AS $file) {
    ...
    if($_GET['file'] == $file) {
        ...
        $files_content .= '<li class="active"><a href="'.URL_NOQUERY.'?id='.$_GET['id'].'&file='.$file.'">'.$file.'</a></li>';

    } else {

        $files_content .= '<li><a href="'.URL_NOQUERY.'?id='.$_GET['id'].'&file='.$file.'">'.$file.'</a></li>';
...
$edit_plugin_content = '<select id="plugin_select" class="select" style="width: 170px" name="id">';
...
$template_page_menu[] = array('title'=>$PMDR->getLanguage('admin_plugins_edit'),'content'=>$edit_plugin_content);
