    ...
    if($campaign['type'] == 'listings') {
        ...
        if($category_count > 1) {
            ...
            if($PMDR->getConfig('category_select_type') == 'tree_select' OR $PMDR->getConfig('category_select_type') == 'tree_select_multiple') {
                ...
                $form->addField('categories',$PMDR->getConfig('category_select_type'),array('label'=>$PMDR->getLanguage('admin_email_campaigns_categories'),'fieldset'=>'filter','first_option'=>'','options'=>$db->GetAssoc("SELECT id, title, level FROM ".T_CATEGORIES." WHERE ID != 1 ORDER BY left_"),'help'=>$PMDR->getLanguage('admin_mail_help_categories')));
        ...
        $form->addField('no_order','checkbox',array('label'=>$PMDR->getLanguage('admin_email_campaigns_no_order'),'fieldset'=>'filter'));

        $form->addField('unconfirmed_email','checkbox',array('label'=>$PMDR->getLanguage('admin_email_campaigns_unconfirmed_email'),'fieldset'=>'filter'));
