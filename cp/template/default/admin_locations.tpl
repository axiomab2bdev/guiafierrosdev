    ...
    $("#locations_search").keyup(function () {
        ...
        if(locations_search > 2 || (locations_search > 0 && !isNaN($("#locations_search").val()))) {
            $.ajax({ data: ({ action: "admin_locations_search", value: $("#locations_search").val()}), success:
                function(data){
                    var myPopover = $('#locations_search').data('popover');
...
<form class="form-inline"><label><?php echo $lang['admin_locations_quick_search']; ?>:</label> <input id="locations_search" class="span4" type="text" name="locations_search"></form>
<?php echo $content; ?>