    ...
    if(data == null) {
        ...
        $.each(data.results, function(i,record) {
            if(data.type == 'user_ip') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_users.php?field=logged_ip&keyword='+record.logged_ip+'">'+record.logged_ip+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            } else if(data.type == 'user_name') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_users.php?compare_type=equal&field=user_first_name&field2=user_last_name&keyword='+htmlspecialchars(record.user_first_name)+'&keyword2='+htmlspecialchars(record.user_last_name)+'">'+htmlspecialchars(record.user_first_name)+' '+htmlspecialchars(record.user_last_name)+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            } else if(data.type == 'user_phone') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_users.php?field=user_phone&keyword='+htmlspecialchars(record.user_phone)+'">'+htmlspecialchars(record.user_phone)+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            } else if(data.type == 'listing_title') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_listings.php?title='+htmlspecialchars(record.title)+'">'+htmlspecialchars(record.title)+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            } else if(data.type == 'listing_www') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_listings.php?www='+htmlspecialchars(record.www)+'">'+htmlspecialchars(record.www)+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            } else if(data.type == 'listing_phone') {
                $("#duplicate_content_results").append('<a href="<?php echo BASE_URL_ADMIN; ?>/admin_listings.php?phone='+htmlspecialchars(record.phone)+'">'+htmlspecialchars(record.phone)+'</a> (<?php echo $lang['admin_maintenance_duplicates_count']; ?>: '+record.count+')<br />');
            }
        });
    }
    $("#loading_image").hide();
