    ...
    $("#categories_search").keyup(function () {
        ...
        if(categories_search > 2 || (categories_search > 0 && !isNaN($("#categories_search").val()))) {
            $.ajax({ data: ({ action: "admin_categories_search", value: $("#categories_search").val()}), success:
                function(data){
                    var myPopover = $('#categories_search').data('popover');
...
<h1 style="clear: left"><?php echo $title; ?></h1>
<div class="form-inline"><label class="control-label" for="categories_search"><?php echo $lang['admin_categories_quick_search']; ?>:</label> <input id="categories_search" class="span4" type="text" name="categories_search"></div>
<?php echo $content; ?>