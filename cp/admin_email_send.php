...
if($_GET['user_id']) {
    $emails = $db->GetAssoc("SELECT id, CONCAT(COALESCE(NULLIF(TRIM(CONCAT(user_first_name,' ',user_last_name)),''),login),' &lt;',user_email,'&gt;') FROM ".T_USERS." WHERE id IN (".$PMDR->get('Cleaner')->clean_db($_GET['user_id']).")");
}
