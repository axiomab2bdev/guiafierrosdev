...
if(!isset($_GET['action'])) {
    ...
    $form->addField('type','select',array('label'=>'Import','fieldset'=>'information','value'=>$_POST['type'],'options'=>array(''=>'Select Import Type','listings'=>'Listings','categories'=>'Categories','locations'=>'Locations'),'help'=>$PMDR->getLanguage('admin_import_help_type')));
    ...
    foreach($field_list as $key=>$field) {
        ...
        $values['custom_'.$field['id']] = array_intersect($scan_fields_lookup,array($field['name']));
    ...
    var importOnComplete = function(data) {
        ...
        if(data.percent == 100) {
            ...
            addMessage("success","<p>'.$PMDR->getLanguage('admin_import_complete').'</p><p><a class=\"btn btn-small\" href=\"admin_import.php?action=download&id="+data.id+"\">Download Full Log</a></p>","status_container");
    ...
    var importStart = function(start) {
        ...
        $.ajax({ data: ({ action: "admin_import", id: '.$_GET['id'].' }), success: importOnComplete, dataType: "json"});
    ...
    <table class="table table-striped table-bordered">
        ...
        <tr>
            ...
            <td><span id="error_count">0</span> <a id="view_errors" target="_blank" style="display: none" class="btn btn-mini btn-warning" href="'.get_file_url(TEMP_UPLOAD_PATH.'import_'.$_GET['id'].'_errors.txt').'">View Error Log</a></td>
    ...
    <div style="clear: both">
        ...
        <a target="_blank" class="btn" href="'.get_file_url(TEMP_UPLOAD_PATH.'import_'.$_GET['id'].'_log.txt').'">View Log</a>
