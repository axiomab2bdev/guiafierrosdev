...
if(!isset($_GET['action'])) {
    ...
    if($_GET['action'] == 'add') {
        ...
        if(in_array($type,array('listings','reviews','send_message','send_message_friend'))) {
            ...
            $field['categories'] = $db->GetCol("SELECT category_id FROM ".T_CATEGORIES_FIELDS." WHERE field_id=?",array($field['id']));
    ...
    $form->addDependency('options',array('type'=>'display','field'=>'type','value'=>array('checkbox','radio','select','select_multiple')));
    ...
    if($type == 'listings' OR $type == 'users' OR $type == 'reviews') {
    ...
    if(in_array($type,array('listings','reviews','send_message','send_message_friend'))) {
        ...
        $form->addField('categories','tree_select_expanding_checkbox',array('fieldset'=>'field_details','options'=>array('type'=>'category_tree','select_mode'=>3,'bypass_setup'=>true)));
