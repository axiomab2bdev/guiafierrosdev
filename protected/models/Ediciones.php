 ...
 */
class Ediciones extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('title, friendly_url, content_short, content, keywords, meta_title, meta_description, meta_keywords', 'length', 'max'=>255),
	 ...
	 * @return Edition the static model class
	 */
	...
	public function getCategoryId(){
		$categoriesLookup = NewsCategories::model()->findByAttributes(array('friendly_url'=>$this->friendly_url));
		
		return $categoriesLookup->id;
	}

	public function isPortrait(){
		$isPortrait = false;
		$categoriesLookup = NewsCategoriesLookup::model()->findByAttributes(array('blog_id'=>$this->id,'category_id'=>3));
		$count = count($categoriesLookup);
		if($count>0)
			$isPortrait = true;
		return $isPortrait;
	}
