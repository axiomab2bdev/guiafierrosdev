<?php
// Redirect to installation if no license number exists
if($license=='License Key Here') {
...
// Set the license to a global as we can use it many places
define('LICENSE',$license);
unset($license);

// Set the security key, if empty use license
define('SECURITY_KEY',!empty($SECURITY_KEY) ? $SECURITY_KEY : LICENSE);
...
// Turn off magix quotes run time if the function exists on the server
...
// Register globals security descruction
// Remove once we require PHP 5.3.0
$types = array($_GET, $_POST, $_SERVER, $_COOKIE, $_REQUEST, $_FILES, $_ENV);
...
if(!CONSOLE) {
    if(!defined('SSL_ON')) {
        define('SSL_ON',(!empty($BASE_URL_SSL) AND (@PMD_SECTION == 'members' OR @PMD_SECTION == 'admin') ? true : false)); // If SSL is attempted, make all base urls use https
    }
    define('BASE_URL', rtrim(((array_key_exists('HTTPS',$_SERVER) AND $_SERVER['HTTPS'] == 'on') ? $BASE_URL_SSL : $BASE_URL),'/'));
    ...
    define('CDN_URL', (!empty($CDN_URL) ? rtrim(((array_key_exists('HTTPS',$_SERVER) AND $_SERVER['HTTPS'] == 'on') ? $CDN_URL_SSL : $CDN_URL),'/') : BASE_URL));
    define('URL','http'.((isset($_SERVER['HTTPS']) AND $_SERVER['HTTPS'] == 'on') ? 's' : '').'://'.(isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME'].(!in_array($_SERVER['SERVER_PORT'],array(80,443)) ? ':'.$_SERVER['SERVER_PORT'] : '')).urldecode(!empty($_SERVER['HTTP_X_REWRITE_URL']) ? $_SERVER['HTTP_X_REWRITE_URL'] : $_SERVER['REQUEST_URI']));
...
if(DEMO_MODE) {
    $PMDR->get('ErrorHandler')->setFlags(true, true, false, true, true, true, true, true, false);
...
// Set the time zone in MySQL and PHP to UTC
...
if(defined('SSL_ON') AND SSL_ON AND @$_SERVER['HTTPS'] != 'on') {
...
foreach($PMDR->get('Plugins')->plugins AS $plugin) {
   if($plugin != 'email_listing') //14/03/16-MySEO:Se dehabilita el plugin ya que no se esta utilizando 'email_listing' (Función vieja de CMS PhpMyDirectory)
    include(PLUGINS_PATH.$plugin.'/'.$plugin.'.php');
}
unset($plugin);

// Take care of magic quotes
// Remove for PHP 5.3.0
...
} elseif(MOBILE AND $PMDR->getConfig('mobile_template') != '') {
...
if($PMDR->getConfig('meta_title_default') != '') {
    $PMDR->setAdd('page_title',$PMDR->getConfig('meta_title_default'));
} else {
    $PMDR->setAdd('page_title','');
}
