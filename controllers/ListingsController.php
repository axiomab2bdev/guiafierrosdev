...
class ListingsController extends Controller
{
				...
				if($listingForm->validate())
						{
							...
							if($Email!=""){
								...
								ApiController::actionContactAnalityc();
							}
							Yii::app()->theme='guia';
							$this->redirect(array('guia/site/page/view/confirm', 'urll' =>$this->loadModel($id)->friendly_url, 'category_id'=>$listingForm->CategoriId));
	...
	{
		require_once('guia/protected/views/guia/listings/recaptchalib.php');
		$model=new Listings;
		$resp5='';
		//Categorias	
...
public function actionIndex($keyword = '',$location = '')
	...
	public function build_4goldListingsForm($gold_listing, $setCategory)	{
		$output = "";
		$output .= "<div id='proveedores-oro' class='hidden'>";
			$output .= "<div class='gold4-box'>";
				$output .= "<div class='interes-info'>";
					...
					$output .= "<p class='cotizacion-text'>Encuentre la mejor opción cotizando con los proveedores destacados de esta categoría: </p>";
				$output .= "</div>";
					...
					$output .= "<form id='form-proveedores'>";
						$output .= "<div class='one-whole'>";
							foreach ($gold_listing as $key => $value) {
								...
								$output .= "<div class='gold4-provider'>";
						...
						$output .= "<div class='one-whole mt20'>";
							...
							$output .= "<p class='cotizacion-text'>¿Quiere editar el mensaje?</p>";
							$output .= "<textarea value='' name='body' class='gold4-message cotizacion-text-area'> </textarea>";
							
							$output .= "<div class='buttons-container mt10'>";
								$output .= "<input id='goldenListing-submit'value='Enviar Cotización' type='button'>";
							$output .= "</div>";
						$output .= "</div>";
					$output .= "</form>";
				$output .= "</div>";
